...
> 变量和对象：(1)对象并没有直接存储到变量中，Python中`变量更像是给对象起了一个别名`；(2)`变量中存储的不是对象的值，而是对象的id（内存地址）`，使用变量时，实际上就是在`通过对象id在查找对象`；(3)变量中保存的对象，只有在为变量重新赋值时才会改变；(4)变量和变量之间是相互独立的，修改一个变量不会影响另一个变量  
