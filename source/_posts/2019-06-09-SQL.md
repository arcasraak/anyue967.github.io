...
### Basic SQL Commands:
```SQL
SHOW	SELECT    INSERT    GREATE    UPDATE    DELETE 		
```
...
SELECT column_list FROM table_name;
  ...
  eg2. SELECT FirstName FROM customers;
  | FirstName |
  |-----------|
  | John      |
  | David     |
  ...
  eg3. SELECT FirstName,LastName,City FROM customers;
...
# 2)The SQL DISTINCT keyword is used in conjunction with SELECT to eliminate all duplicate records and return only unique ones.(不选择重复的数据).
  ...
  eg2. SELECT DISTINCT City FORM customers;
  |     City    |
  |-------------|
  | NewYork     |
  | Los Angeles |
...
#### INSERT: <a id="insert" target="_self"></a>
```SQL
...
#### UPDATE:UPDATE语句允许我们更改表中的数据。 <a id="update" target="_self"></a>
```SQL
...
#### ALERT TABLE:ALTER TABLE命令用于添加，删除或修改现有表中的列。 <a id="alter" target="_self"></a>
```SQL
eg. ALTER TABLE People ADD DateOfBirth date;
  上述SQL代码People表中添加一个名为DateOfBirth的新列

eg. ALTER TABLE People DROP COLUMN DateOfBirth;
  上述SQL代码删除People表中名为DateOfBirth的列。

eg. DROP TABLE People;

eg. ALTER TABLE People CHANGE FirstName name varchar(100);
eg. RENAME TABLE People TO Users;
  上述SQL代码重命名列
```
